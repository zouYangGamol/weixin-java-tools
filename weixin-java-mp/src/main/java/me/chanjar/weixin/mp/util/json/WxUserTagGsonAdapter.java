...
public class WxUserTagGsonAdapter implements JsonSerializer<WxUserTag>, JsonDeserializer<WxUserTag> {

  @Override
  public JsonElement serialize(WxUserTag group, Type typeOfSrc, JsonSerializationContext context) {
  ...
  @Override
  public WxUserTag deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      WxUserTag group = new WxUserTag();
    ...
    if (groupJson.get("id") != null && !groupJson.get("id").isJsonNull()) {
      group.setId(GsonHelper.getAsPrimitiveInt(groupJson.get("id")));
    }
    if (groupJson.get("count") != null && !groupJson.get("count").isJsonNull()) {
      group.setCount(GsonHelper.getAsPrimitiveInt(groupJson.get("count")));
    }
    return group;
  }
